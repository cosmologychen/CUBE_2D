...
subroutine Green_2D_dyn(Gk,nglobal,n1,n2,apm_soft,apm_range,gridsize)
  use omp_lib
  use parameters
  use, intrinsic :: ieee_arithmetic
  implicit none
  ...
  call system_clock(t1,t_rate)
  print*,'  Initialize 2D dynamic Green''s function'
  print*,'    mesh =',int(nglobal,kind=2)
  print*,'    n1,n2 =',int(n1,kind=2),int(n2,kind=2)
  print*,'    apm_soft, apm_range =',apm_soft,apm_range
  print*,'    gridsize =',gridsize
  ...
  do k1=1,n1
    ...
    do i1=-n_int,n_int
      ...
      if (kmag_n==0) then
        ...
        Rvec_n=kvec_n*(S_soft**2-S_range**2)/kmag_n
      endif
      ur_cn=ur_cn+Uk2_n*Rvec_n
