subroutine kick
   ...
   real,allocatable :: af(:,:)
   real(8) pt,fpp(2),rho_mean
   real rmag,rmag2,rvec(2),bpp2
   real dx(2,p+1),l2(2),dv(2),xpos(2)
   real :: a_local  ! one_run_lightcone: 局部尺度因子
   integer,allocatable :: ll(:),hoc(:,:)!,ll2(:)

   real :: x1(2)
   integer :: x_wrapped1(2)! ,pp_count(nc,nc), pp_in(sim%np), pp_neighbor(4,sim%np)
   ...
   if (one_run_lightcone) then
      rho_mean = sum(rho1)/(nc*nc)
      do j = 1, nw
         do i = 1, nw
            rho1(i, j) = (rho1(i, j) - rho_mean) / D_grid(i*ratio_cs-ratio_cs/2, j*ratio_cs-ratio_cs/2)
         enddo
      enddo
   endif
   ...
   !$omp paralleldo default(shared) schedule(dynamic)&
   !$omp& private(ip,xpos,idx,dx,l2,i1,i2,a_local)
   do ip=1,sim%np
      ! iteam=omp_get_thread_num()+1
      xpos=xp(:,ip)/ratio_cs
      idx(:,2)=floor(xpos)+1
      ...
      ! one_run_lightcone: 使用a_grid替代a_mid
      if (one_run_lightcone) then
         idx(:,2)=floor(xp(:,ip))+1
         a_local = a_grid(idx(1,2), idx(2,2)) * D_grid(idx(1,2), idx(2,2))
      else
         a_local = a_mid
      endif
      vp(:,ip)=vp(:,ip)+dv*a_local*dt*G_grid
   enddo ! ip
   !$omp endparalleldo
   deallocate(force)
   call toc(11)
   ...
   !$omp& private(itile,iteam,ix,iy,i,j,ip,xpos,idx,dx,i1,i2,l2,dv,tshift,a_local)  !!num_threads(1)
   do itile=0,nnt**2-1
      iteam=omp_get_thread_num()+1
      itx = itile/nnt
      ity = mod(itile,nnt)
      ...
      ! print*, iteam,itx,ity,'rho2',rho2(1,1,iteam),rho2(ngp,ngp,iteam)!maxval(rho2(1-ngb:ngp+ngb,1-ngb:ngp+ngb,iteam)),minval(rho2(1-ngb:ngp+ngb,1-ngb:ngp+ngb,iteam))
      if (one_run_lightcone) then
         rho_mean = sum(rho2(1-ngb:ngp+ngb,1-ngb:ngp+ngb,iteam))/(ngt*ngt)
         do j = 1-ngb,ngp+ngb
            do i = 1-ngb,ngp+ngb
               tshift = [i,j]+[ix,iy]*ratio_cs
               tshift = modulo(tshift-1,ng)+1
               rho2(i, j,iteam) = (rho2(i, j,iteam)-rho_mean) / D_grid(tshift(1), tshift(2))
            enddo
         enddo
      endif
      ! print*, iteam,itx,ity,'rho2/D',rho2(1,1,iteam),rho2(ngp,ngp,iteam)!minval(rho2(1-ngb:ngp+ngb,1-ngb:ngp+ngb,iteam))
   ...
   if (PP) then ! =====================================================
      ...
      ! pp_in = 0
      !$omp paralleldo default(shared) schedule(dynamic)&
      !$omp private(j,i,ip1,ip2,rvec,rmag2,rmag,fpp)
      do j=1,nc
         do i=1,nc
            ip1=hoc(i,j)
            do while(ip1/=0) ! particle A
               ip2=ll(ip1)
               ! pp_count(i,j) = pp_count(i,j) + 1
               do while (ip2/=0) ! particle B in the same cell
                  rvec=xp(:,ip2)-xp(:,ip1); rmag2=sum(rvec**2)
                  if (rmag2<bpp2) then
                     rmag = sqrt(rmag2)
                     fpp=((F_ra(rmag,app)-F_ra(rmag,apm2))/rmag)*rvec
                     af(:,ip1)=af(:,ip1)+fpp
                     af(:,ip2)=af(:,ip2)-fpp
                     ! pp_in(ip1) = pp_in(ip1) + 1
                     ! pp_in(ip2) = pp_in(ip2) + 1
                  endif
                  ip2=ll(ip2)
               enddo ! particle B
               ip1=ll(ip1)
      ...
      enddo
      !$omp endparalleldo
      ! print*,'pp_count in',maxval(pp_in),minval(pp_in)
      ...
      do ipeny=1,2
         do ipenx=1,4
            !$omp paralleldo default(shared) schedule(dynamic)&!! num_threads(1)&
            !$omp& private(i_iter,iy,ix,j,i,ip1,i_neighbor,idl,ip2,rvec,rmag2,rmag,fpp,tshift)
            do i_iter=0,n_iter-1
               ...
               ip1=hoc(i,j)
               if (ip1==0) cycle
               do i_neighbor=1,4 ! neighbor cells
                  tshift = 0
                  ...
                  do while(ip1/=0) ! particle A
                     ...
                     do while (ip2/=0) ! particle B
                        rvec=xp(:,ip2)+tshift-xp(:,ip1); rmag2=sum(rvec**2)
                        if (0.<rmag2 .and. rmag2<bpp2) then
                           rmag = sqrt(rmag2)
                           fpp=((F_ra(rmag,app)-F_ra(rmag,apm2))/rmag)*rvec
                           af(:,ip1)=af(:,ip1)+fpp
                           af(:,ip2)=af(:,ip2)-fpp
                           ! pp_neighbor(i_neighbor,ip1) = pp_neighbor(i_neighbor,ip1) + 1
                           ! pp_neighbor(i_neighbor,ip2) = pp_neighbor(i_neighbor,ip2) + 1
                        endif
                        ip2=ll(ip2)
                     enddo
                     ip1=ll(ip1)
         ...
         enddo ! ipenx
      enddo ! ipeny
      ! print*,'pp_neighbor     1',maxval(pp_neighbor(1,:)),minval(pp_neighbor(1,:))
      ! print*,'pp_neighbor     2',maxval(pp_neighbor(2,:)),minval(pp_neighbor(2,:))
      ! print*,'pp_neighbor     3',maxval(pp_neighbor(3,:)),minval(pp_neighbor(3,:))
      ! print*,'pp_neighbor     4',maxval(pp_neighbor(4,:)),minval(pp_neighbor(4,:))
      ! ! if (istep > 20 ) stop
      ! ! stop

      ! one_run_lightcone: 使用a_grid替代a_mid
      if (one_run_lightcone) then
         !$omp paralleldo default(shared) private(ip,ix,iy,idx,xpos,a_local)
         do ip=1,sim%np
            ! 计算粒子所在的格点索引
            xpos=xp(:,ip)
            idx(1,2)=floor(xpos(1))+1
            idx(2,2)=floor(xpos(2))+1
            vp(:,ip)=vp(:,ip)+af(:,ip)*a_grid(idx(1,2), idx(2,2))*dt*G_grid*sim%mass_p_cdm
         enddo
         !$omp endparalleldo
      else
   ...
   ! if (maxval(vmax)<0.01) final_step=.true.
